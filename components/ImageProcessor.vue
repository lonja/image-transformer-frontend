<template>
  <div>
    <el-form :model="formData" ref="rotateForm" :rules="validationRules" label-width="65px">
      ...
      <el-form-item label="Angle" prop="angle">
        <el-select v-model="formData.angle" placeholder="select angle to rotate image">
          <el-option
      ...
      <el-form-item>
        <el-button type="success" @click="onSubmit">Rotate</el-button>
      </el-form-item>
    </el-form>
    ...
    data () {
      const validateFiles = (rule, value, callback) => {
        ...
        if (!this.formData.files || this.formData.files.length === 0) {
          callback(new Error('Please select files to rotate'))
        } else {
          callback()
        }
      }
    ...
    methods: {
      onSubmit: function () {
        ...
        this.$refs.rotateForm.validate((valid) => {
          if (valid) {
            this.loading = true
            rotate(this.formData.files).then(files => {
